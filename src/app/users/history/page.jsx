  ...
  if (initialLoad) {
    return (
      <div className="p-8 max-w-4xl mx-auto flex flex-col items-center justify-center min-h-[50vh]">
        <div className="animate-spin w-10 h-10 border-4 border-[#33D6A6] border-t-transparent rounded-full mb-4"></div>
  ...
  return (
    <div className="p-4 sm:p-8 max-w-5xl mx-auto space-y-6 relative min-h-[70vh]">
      ...
      <div>
        <button 
          onClick={() => router.back()}
          className="flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-[#33D6A6] transition-colors mb-4"
        >
          ...
          {reports.map((report, index) => (
            <Link href={`/laporan/${report.id}`} key={report.id}>
              <motion.div 
                ...
                className="group cursor-pointer bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 hover:border-[#33D6A6] hover:shadow-md transition-all flex flex-col sm:flex-row w-full min-h-[120px]"
              >
                {/* Thumbnail Image */}
